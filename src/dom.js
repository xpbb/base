...
xp.dom = function() {
	...
	return {
		 ...
		 */
		px : function(v){
			if(typeof v === 'string' && ~v.indexOf("px")){
		...
		css : function(el, name, val) {
			...
			var me = xp.dom, el = me.id(el), name = me._camelCssName(name), v, css = xp.css.css;
			if (el) {
				if(val){
					return css(el, name, val);
				}else{
					return me.px(css(el, name, val));
				}
			...
			var me = xp.dom,
				...
				set = me.setStyle,
				parseVal = function(v){
					if(typeof v == "string" && v.indexOf("px") > -1){
			...
			if(!val){
				if(typeof name === "string"){
					return parseVal(get(el, name));
				}else{
					var css = {}, p;
					for( p in name ){
						css[p] = parseVal(get(el, p));
					}
					return css;
				}
				...
				if(typeof name === "string"){
					set(el, name, getVal(val));
				}else{
					//如果是对象
					for(var p in name){
						set(el, p, getVal(val[p]));
					}
				}
